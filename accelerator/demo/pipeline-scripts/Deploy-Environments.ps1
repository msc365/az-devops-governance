...
param (
    [Parameter()]
    [string]$TemplateFile = 'src/res/pipeline/environment/main.ps1',

    [Parameter()]
    [string]$TemplateParameterFile = 'params/devops/pipeline_environment.main.parameters.json',
    ...
    try {
        ...
        foreach ($project in $projects) {
            ...
            if ($null -eq $environments -or $environments.Count -eq 0) {
                throw "Project '$projectName' does not define any environments."
            }

            Write-Verbose "Processing $($environments.Count) environment(s)..."
